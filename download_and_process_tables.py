"""
When run, this script downloads all the base tables,
and creates the summary csv files that go in the website.
"""
...
from create_table import create_base_table
...
def compute_pmo_benchmark_table(
    ...
    summary_avg, summary_std = summary_per_function(
        df, normalized_per_row=False, n_dimensions=n_dimensions, use_tex=use_tex
    )
...
if __name__ == "__main__":
    ...
    for n_dimensions in [2, 128]:
        ...
        table_.to_csv(ROOT_DIR / "csv" / f"pmo_benchmark_task_{n_dimensions}d.csv")

        row_for_final_table = table_.loc["Sum (normalized per row)"]
        row_for_final_table.name = f"PMO {n_dimensions}D"
        model_vs_benchmark_table_columns.append(row_for_final_table)
