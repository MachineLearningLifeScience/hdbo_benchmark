...
def tensor_to_sim_level(tensor_levels: Tensor, level_size: int = 14) -> list[list[int]]:
    ...
    lvls_with_padding_as_list: list[list[int]] = lvls_with_padding.tolist()

    return lvls_with_padding_as_list
...
def add_padding_to_level(level: np.ndarray, n_padding: int = 1) -> np.ndarray:
    ...
    padding[-1, :] = 0  # Adding the ground.
    level_with_padding: np.ndarray = np.concatenate((padding, level), axis=1)
...
def clean_level(level: np.ndarray) -> list[list[int]]:
    ...
    level_as_list: list[list[int]] = level.tolist()

    return level_as_list
