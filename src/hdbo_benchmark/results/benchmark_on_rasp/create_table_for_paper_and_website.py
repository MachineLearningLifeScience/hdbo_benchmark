...
def summary_per_function(
    ...
    if solvers is not None:
        df = df[df["solver_name"].isin(solvers)]
    ...
    for function_name in df["function_name"].unique():
        for solver_name in df["solver_name"].unique():
            ...
            slice_df = slice_df[slice_df["seed"].isin([1, 2, 3, 4, 5])]
            if (
...
def compute_rasp_benchmark_table(
    df, n_dimensions: int, use_tex: bool = True, normalized: bool = False
) -> pd.DataFrame:
    solver_name_but_pretty = compute_pretty_names(n_dimensions, use_tex=use_tex)
...
if __name__ == "__main__":
    ...
    tags = [
        "RaSPExperiments-2024-08-27",
        "RaSPExperiments-2024-08-28",
        "RaSPExperiments-2024-08-29",
    ]
    for n_dimensions in [32]:
        ...
        # plot_heatmap(df, normalized=normalized, n_dimensions=n_dimensions)
        table_ = compute_rasp_benchmark_table(
            df, n_dimensions=n_dimensions, use_tex=False, normalized=False
        )
        table_.to_csv(f"table_rasp_{n_dimensions}.csv")
        table_normalized = compute_rasp_benchmark_table(
            df, n_dimensions=n_dimensions, use_tex=False, normalized=True
        )
        table_normalized.to_csv(f"table_rasp_normalized_{n_dimensions}.csv")
